...
typedef struct
{
    ...
    uint32_t
    pageSize;                   /**< Size of a page, usually denotes maximum write size in bytes for a single write operation */
    uint32_t
    maxReadSize;                /**< Maximum read size in bytes for a single read operation */
...
typedef struct
{
    ...
    SPIFI_ERR_T (*init)(struct SPIFI_HANDLE
                        *);                                                             /**< Initialize SPIFI device */
    SPIFI_ERR_T (*deInit)(struct SPIFI_HANDLE
                          *);                                                           /**< De-initialize SPIFI device */
    SPIFI_ERR_T (*unlockDevice)(struct SPIFI_HANDLE
                                *);                                             /**< Full device unlock */
    SPIFI_ERR_T (*lockDevice)(struct SPIFI_HANDLE
                              *);                                                   /**< Full device lock */
    SPIFI_ERR_T (*unlockBlock)(struct SPIFI_HANDLE *,
                               uint32_t);                            /**< Unlock a block */
    SPIFI_ERR_T (*lockBlock)(struct SPIFI_HANDLE *,
                             uint32_t);                              /**< Lock a block */
    SPIFI_ERR_T (*eraseAll)(struct SPIFI_HANDLE
                            *);                                                     /**< Full device erase */
    SPIFI_ERR_T (*eraseBlock)(struct SPIFI_HANDLE *,
                              uint32_t);                             /**< Erase a block by block number */
    SPIFI_ERR_T (*eraseSubBlock)(struct SPIFI_HANDLE *,
                                 uint32_t);                      /**< Erase a sub-block by block number */
    SPIFI_ERR_T (*pageProgram)(struct SPIFI_HANDLE *, uint32_t, const uint32_t *,
                               uint32_t);        /**< Program up to a page of data at an address */
    SPIFI_ERR_T (*read)(struct SPIFI_HANDLE *, uint32_t, uint32_t *,
                        uint32_t); /**< Read an address range */
    SPIFI_ERR_T (*setOpts)(struct SPIFI_HANDLE *, uint32_t,
                           bool);                      /**< Set or Unset driver options */
    SPIFI_ERR_T (*reset)(struct SPIFI_HANDLE
                         *);                                                            /**< Reset SPIFI device */

    /* Info query functions */
    uint32_t (*getMemModeCmd)(struct SPIFI_HANDLE *,
                              bool);                                     /**< returns memoryMode cmd */
    const char *(*getDevName)(struct SPIFI_HANDLE
                              *);                                                   /**< Returns the device name */
    uint32_t (*getInfo)(struct SPIFI_HANDLE *,
                        SPIFI_INFO_ID_T);                            /**< Returns info on the device */
} SPIFI_DEV_T;
...
typedef struct SPIFI_HANDLE
{
    ...
    void
    *pDevData;          /**< Pointer to device context (used by device functions) */
} SPIFI_HANDLE_T;
...
typedef struct
{
    ...
    uint32_t
    prvDataSize;                    /**< Number of bytes needed for driver private data area allocation */
    uint32_t (*pPrvDevDetect)(uint32_t
                              baseAddr);   /**< Pointer to device specific detection function */
    SPIFI_ERR_T (*pPrvDevSetup)(SPIFI_HANDLE_T *pHandle, uint32_t spifiCtrlAddr,
                                uint32_t baseAddr);    /**< Pointer to device specific device initialization */
    SPIFI_ERR_T (*pPrvDevRegister)(void *pFamily,
                                   SPIFI_DEV_DATA_T *pDevData); /**< Pointer to device specific register device function */
} SPIFI_DEVDESC_T;
