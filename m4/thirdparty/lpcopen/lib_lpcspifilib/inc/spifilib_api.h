 ...
 */
SPIFI_HANDLE_T *spifiInitDevice(void *pMem, uint32_t sizePMem, uint32_t spifiCtrlAddr,
                                uint32_t baseAddr);
 ...
 */
bool spifiDevGetMemoryMode(SPIFI_HANDLE_T *pSpifi);
...
{
    return pHandle->pDev->unlockDevice(pHandle);
}
...
{
    return pHandle->pDev->lockDevice(pHandle);
}
...
{
    return pHandle->pDev->unlockBlock(pHandle, block);
}
...
{
    return pHandle->pDev->lockBlock(pHandle, block);
}
 ...
 */
STATIC INLINE SPIFI_ERR_T spifiDevRead(SPIFI_HANDLE_T *pHandle, uint32_t addr, uint32_t *readBuff,
                                       uint32_t bytes)
{
 ...
 */
STATIC INLINE const char *spifiDevGetFamilyName(SPIFI_HANDLE_T *pHandle)
{
    return pHandle->pDev->getDevName(pHandle);
}
 ...
 */
STATIC INLINE uint32_t spifiDevGetInfo(SPIFI_HANDLE_T *pHandle, SPIFI_INFO_ID_T infoId)
{
    return pHandle->pDev->getInfo(pHandle, infoId);
}
 ...
 */
SPIFI_ERR_T spifiProgram(SPIFI_HANDLE_T *pHandle, uint32_t addr, const uint32_t *writeBuff,
                         uint32_t bytes);
