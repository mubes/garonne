...
typedef struct
{
    ...
    uint8
    init_xtrim;         // initial XTAL trim value read from OTP (or defaulted to mid-range if OTP not programmed)
...
int dwt_initialise(uint16 config)
{
    ...
    _dwt_enableclocks(
        FORCE_SYS_XTI); // NOTE: set system clock to XTI - this is necessary to make sure the values read by _dwt_otpread are reliable

    // Configure the CPLL lock detect
    dwt_write8bitoffsetreg(EXT_SYNC_ID, EC_CTRL_OFFSET, EC_CTRL_PLLLCK);

    // Read OTP revision number
    otp_addr = _dwt_otpread(XTRIM_ADDRESS) &
...
void dwt_configure(dwt_config_t *config)
{
    ...
    assert((config->txPreambLength == DWT_PLEN_64) || (config->txPreambLength == DWT_PLEN_128)
           || (config->txPreambLength == DWT_PLEN_256)
           || (config->txPreambLength == DWT_PLEN_512) || (config->txPreambLength == DWT_PLEN_1024)
    ...
    dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET,
                           SYS_CTRL_TXSTRT | SYS_CTRL_TRXOFF); // Request TX start and TRX off at the same time
} // end dwt_configure()
...
void dwt_writetxfctrl(uint16 txFrameLength, uint16 txBufferOffset, int ranging)
{
    ...
    uint32 reg32 = dw1000local.txFCTRL | txFrameLength | (txBufferOffset << TX_FCTRL_TXBOFFS_SHFT) |
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics)
{
    ...
    diagnostics->rxPreamCount = (dwt_read32bitreg(RX_FINFO_ID) & RX_FINFO_RXPACC_MASK) >>
...
uint32 dwt_readtxtimestamplo32(void)
{
    return dwt_read32bitreg(
               TX_TIME_ID); // Read TX TIME as a 32-bit register to get the 4 lower bytes out of 5
}
...
{
    return dwt_read32bitreg(
               RX_TIME_ID); // Read RX TIME as a 32-bit register to get the 4 lower bytes out of 5
}
...
{
    uint8 header[3]= {0} ; // Buffer to compose header in
    ...
    if (index == 0) // For index of 0, no sub-index is required
        {
            header[cnt++] = 0x80 | recordNumber
                            ; // Bit-7 is WRITE operation, bit-6 zero=NO sub-addressing, bits 5-0 is reg file id
        }
    else
        {
#ifdef DWT_API_ERROR_CHECK
            assert((index <= 0x7FFF)
                   && ((index + length) <= 0x7FFF)); // Index and sub-addressable area are limited to 15-bits.
#endif
    ...
    if (index == 0) // For index of 0, no sub-index is required
    ...
    else
        {
            ...
            assert((index <= 0x7FFF)
                   && ((index + length) <= 0x7FFF)); // Index and sub-addressable area are limited to 15-bits.
#endif
            header[cnt++] = (uint8)(0x40 | recordNumber)
...
void dwt_otpread(uint32 address, uint32 *array, uint8 length)
{
    ...
    _dwt_enableclocks(
        FORCE_SYS_XTI); // NOTE: Set system clock to XTAL - this is necessary to make sure the values read by _dwt_otpread are reliable
...
uint32 dwt_otpwriteandverify(uint32 value, uint16 address)
{
    ...
    _dwt_otpsetmrregs(
        0); // Setting OTP mode register for low RM read - resetting the device would be alternative
...
void dwt_configuresleepcnt(uint16 sleepcnt)
{
    ...
    dwt_write8bitoffsetreg(AON_ID, AON_CFG0_OFFSET,
                           0x00); // NB: this write change the default LPCLKDIVA value which is not used anyway.
    dwt_write8bitoffsetreg(AON_ID, AON_CFG1_OFFSET, 0x00);
    ...
    if(dwt_readdevid() != DWT_DEVICE_ID) // Device was in deep sleep (the first read fails)
        {
            ...
            dwt_readfromdevice(0x0, 0x0, length,
...
void _dwt_configlde(int prfIndex)
{
    ...
    if(prfIndex)
        {
            dwt_write16bitoffsetreg( LDE_IF_ID, LDE_CFG2_OFFSET,
...
{
    uint16 reg = ((ops_sel << OTP_SF_OPS_SEL_SHFT) & OTP_SF_OPS_SEL_MASK) |
                 OTP_SF_OPS_KICK; // Select defined OPS table and trigger its loading
...
{
    return (dwt_read8bitoffsetreg(SYS_STATUS_ID,
                                  SYS_STATUS_OFFSET) & SYS_STATUS_IRQS); // Reading the lower byte only is enough for this operation
}
...
void dwt_isr(void)
{
    uint32 status = dw1000local.cbData.status = dwt_read32bitreg(
                        SYS_STATUS_ID); // Read status register low 32bits
...
{
    dwt_write32bitoffsetreg(DX_TIME_ID, 1,
                            starttime); // Write at offset 1 as the lower 9 bits of this register are ignored
    ...
    if (mode & DWT_START_TX_DELAYED)
        {
            ...
            dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, temp);
            checkTxOK = dwt_read16bitoffsetreg(SYS_STATUS_ID,
                                               3); // Read at offset 3 to get the upper 2 bytes out of 5
            if ((checkTxOK & SYS_STATUS_TXERR) ==
                    0) // Transmit Delayed Send set over Half a Period away or Power Up error (there is enough time to send but not to power up individual blocks).
                {
...
void dwt_forcetrxoff(void)
{
    ...
    dwt_write32bitreg(SYS_STATUS_ID,
                      (SYS_STATUS_ALL_TX | SYS_STATUS_ALL_RX_ERR | SYS_STATUS_ALL_RX_TO | SYS_STATUS_ALL_RX_GOOD));
...
void dwt_syncrxbufptrs(void)
{
    uint8  buff ;
    // Need to make sure that the host/IC buffer pointers are aligned before starting RX
    buff = dwt_read8bitoffsetreg(SYS_STATUS_ID,
                                 3); // Read 1 byte at offset 3 to get the 4th byte out of 5

    if((buff & (SYS_STATUS_ICRBP >> 24)) !=     // IC side Receive Buffer Pointer
            ((buff & (SYS_STATUS_HSRBP>>24)) << 1) ) // Host Side Receive Buffer Pointer
        {
            dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET ,
                                   0x01) ; // We need to swap RX buffer status reg (write one to toggle internally)
        }
}
...
int dwt_rxenable(int mode)
{
    ...
    if (mode & DWT_START_RX_DELAYED) // check for errors
        {
            temp1 = dwt_read8bitoffsetreg(SYS_STATUS_ID,
                                          3); // Read 1 byte at offset 3 to get the 4th byte out of 5
            if ((temp1 & (SYS_STATUS_HPDWARN >> 24)) !=
                    0) // if delay has passed do immediate RX on unless DWT_IDLE_ON_DLY_ERR is true
                {
...
void dwt_readeventcounters(dwt_deviceentcnts_t *counters)
{
    ...
    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID,
                                  EVC_FFR_OFFSET); // Overruns (31-16), address errors (15-0)
...
void dwt_configcontinuousframemode(uint32 framerepetitionrate)
{
    ...
    //
    dwt_write8bitoffsetreg(DIG_DIAG_ID, DIAG_TMC_OFFSET,
                           (uint8)(DIAG_TMC_TX_PSTM)); // Turn the tx power spectrum test mode - continuous sending of frames
}
...
uint16 dwt_readtempvbat(uint8 fastSPI)
{
    ...
    if(fastSPI == 1)
        ...
        {
            _dwt_enableclocks(
                FORCE_SYS_XTI); // NOTE: set system clock to XTI - this is necessary to make sure the values read are reliable
            // Read voltage and temperature.
            dwt_readfromdevice(TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET,2,wr_buf);
            // Default clocks (ENABLE_ALL_SEQ)
            _dwt_enableclocks(ENABLE_ALL_SEQ); // Enable clocks for sequencing
