...
typedef struct tskTaskControlBlock
{
    ...
    *pxTopOfStack;  /*< Points to the location of the last item placed on the tasks stack.  THIS MUST BE THE FIRST MEMBER OF THE TCB STRUCT. */
    ...
    char
    pcTaskName[ configMAX_TASK_NAME_LEN ];/*< Descriptive name given to the task when created.  Facilitates debugging only. */ /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
...
extern void vApplicationGetIdleTaskMemory( StaticTask_t **ppxIdleTaskTCBBuffer,
        StackType_t **ppxIdleTaskStackBuffer, uint16_t *pusIdleTaskStackSize );
#endif
 ...
 */
static void prvInitialiseTCBVariables( TCB_t * const pxTCB, const char * const pcName,
                                       UBaseType_t uxPriority, const MemoryRegion_t * const xRegions,
...
static TCB_t *prvAllocateTCBAndStack( const uint16_t usStackDepth,
                                      StackType_t * const puxStackBuffer, TCB_t * const pucTCBBuffer ) PRIVILEGED_FUNCTION;
...
void vTaskStartScheduler( void )
{
    ...
    xReturn = xTaskGenericCreate( prvIdleTask, "IDLE", usIdleTaskStackSize, ( void * ) NULL,
                                  ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle, pxIdleTaskStackBuffer,
                                  pxIdleTaskTCBBuffer,
...
TaskHandle_t xTaskGetTaskHandle( const char
...
static void prvInitialiseTCBVariables( TCB_t * const pxTCB, const char * const pcName,
                                       UBaseType_t uxPriority, const MemoryRegion_t * const xRegions,
                                       const uint16_t
                                       usStackDepth ) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
{
    UBaseType_t x;
                                      ...
                                      StackType_t * const puxStackBuffer, TCB_t * const pxTaskBuffer )
{
        ...
        if( pxNewTCB != NULL )
            {
                ...
                pxNewTCB->pxStack = ( StackType_t * ) pvPortMallocAligned( ( ( ( size_t ) usStackDepth ) * sizeof(
                                        StackType_t ) ),
...
#else /* portSTACK_GROWTH */
    {
        ...
        /* Allocate space for the stack used by the task being created. */
        pxStack = ( StackType_t * ) pvPortMallocAligned( ( ( ( size_t ) usStackDepth ) * sizeof(
                      StackType_t ) ),
                  puxStackBuffer ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
        ...
        if( pxStack != NULL )
            {
                ...
                pxNewTCB = ( TCB_t * ) pvPortMallocAligned( sizeof( TCB_t ),
                           pxTaskBuffer ); /*lint !e961 MISRA exception as the casts are only redundant for some paths. */
