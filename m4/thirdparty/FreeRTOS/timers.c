...
typedef struct tmrTimerControl
{
    ...
    *pvTimerID;         /*<< An ID to identify the timer.  This allows the timer to be identified when the same callback is used for multiple timers. */
    ...
    uint8_t
    ucStaticallyAllocated; /*<< Set to pdTRUE if the timer was created from a StaticTimer_t structure, and pdFALSE if the timer structure was allocated dynamically. */
#endif
} xTIMER;
...
extern void vApplicationGetTimerTaskMemory( StaticTask_t **ppxTimerTaskTCBBuffer,
        StackType_t **ppxTimerTaskStackBuffer, uint16_t *pusTimerTaskStackSize );
...
TimerHandle_t xTimerGenericCreate( const char * const pcTimerName,
                                   ...
                                   StaticTimer_t
                                   *pxTimerBuffer ) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
{
    Timer_t *pxNewTimer;
    ...
    if( xTimerPeriodInTicks == ( TickType_t ) 0U )
    ...
    else
        {
            ...
            if( pxTimerBuffer == NULL )
                ...
                {
                    pxNewTimer = ( Timer_t * )
                                 pxTimerBuffer; /*lint !e740 Unusual cast is ok as the structures are designed to have the same alignment, and the size is checked by an assert. */
                }
...
BaseType_t xTimerIsTimerActive( TimerHandle_t xTimer )
{
    ...
    taskENTER_CRITICAL();
    {
        ...
        xTimerIsInActiveList = ( BaseType_t ) !( listIS_CONTAINED_WITHIN( NULL,
