 ...
 */
static Queue_t *prvAllocateQueueMemory( const UBaseType_t uxQueueLength,
                                        const UBaseType_t uxItemSize, uint8_t **ppucQueueStorage,
...
static Queue_t *prvAllocateQueueMemory( const UBaseType_t uxQueueLength,
                                        const UBaseType_t uxItemSize, uint8_t **ppucQueueStorage, StaticQueue_t *pxStaticQueue )
{
    Queue_t *pxNewQueue;
...
#else
    {
        if( pxStaticQueue == NULL )
        ...
        else
            {
                ...
                pxNewQueue = ( Queue_t * )
                             pxStaticQueue; /*lint !e740 Unusual cast is ok as the structures are designed to have the same alignment, and the size is checked by an assert. */
...
QueueHandle_t xQueueCreateMutex( const uint8_t ucQueueType, StaticQueue_t *pxStaticQueue )
{
    ...
    pxNewQueue = ( Queue_t * ) xQueueGenericCreate( uxMutexLength, uxMutexSize, NULL, pxStaticQueue,
                 ucQueueType );
...
BaseType_t xQueueGiveMutexRecursive( QueueHandle_t xMutex )
{
    ...
    if( pxMutex->pxMutexHolder == ( void * )
            xTaskGetCurrentTaskHandle() ) /*lint !e961 Not a redundant cast as TaskHandle_t is a typedef. */
        {
...
BaseType_t xQueueTakeMutexRecursive( QueueHandle_t xMutex, TickType_t xTicksToWait )
{
    ...
    if( pxMutex->pxMutexHolder == ( void * )
            xTaskGetCurrentTaskHandle() ) /*lint !e961 Cast is not redundant as TaskHandle_t is a typedef. */
        {
            ( pxMutex->u.uxRecursiveCallCount )++;
            xReturn = pdPASS;
        }
        ...
        const UBaseType_t uxInitialCount, StaticQueue_t *pxStaticQueue )
{
    ...
    xHandle = xQueueGenericCreate( uxMaxCount, queueSEMAPHORE_QUEUE_ITEM_LENGTH, NULL, pxStaticQueue,
    ...
    if( pxQueue->uxItemSize == ( UBaseType_t ) 0 )
    ...
    else if( xPosition == queueSEND_TO_BACK )
        {
            ( void ) memcpy( ( void * ) pxQueue->pcWriteTo, pvItemToQueue,
                             ( size_t )
                             pxQueue->uxItemSize ); /*lint !e961 !e418 MISRA exception as the casts are only redundant for some ports, plus previous logic ensures a null pointer can only be passed to memcpy() if the copy size is 0. */
    ...
    else
        {
            ( void ) memcpy( ( void * ) pxQueue->u.pcReadFrom, pvItemToQueue,
                             ( size_t )
                             pxQueue->uxItemSize ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
    ...
    if( pxQueue->uxItemSize != ( UBaseType_t ) 0 )
        {
            pxQueue->u.pcReadFrom += pxQueue->uxItemSize;
            if( pxQueue->u.pcReadFrom >=
                    pxQueue->pcTail ) /*lint !e946 MISRA exception justified as use of the relational operator is the cleanest solutions. */
                {
            ...
            ( void ) memcpy( ( void * ) pvBuffer, ( void * ) pxQueue->u.pcReadFrom,
                             ( size_t )
                             pxQueue->uxItemSize ); /*lint !e961 !e418 MISRA exception as the casts are only redundant for some ports.  Also previous logic ensures a null pointer can only be passed to memcpy() when the count is 0. */
        }
}
/*-----------------------------------------------------------*/
...
const char *pcQueueGetQueueName( QueueHandle_t
...
QueueSetHandle_t xQueueCreateSet( const UBaseType_t uxEventQueueLength )
{
    ...
    pxQueue = xQueueGenericCreate( uxEventQueueLength, sizeof( Queue_t * ), NULL, NULL,
                                   queueQUEUE_TYPE_SET );
